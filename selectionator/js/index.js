        ...
        optgroup.options.forEach(function(option, index) {
          ...
          checkbox.addEventListener('change', function(event){
            ...
            if (checkbox.checked) {
              ...
              sellang.push($el.data().value)
              placeholder.nodeValue = "Выбранные языки: " + that.options.checkedItems.length + " из " + $(that.selector).find('input[type="checkbox"]').length;
            } else {
              that.options.checkedItems.pop();
              sellang = sellang.filter(item => item !== $el.data().value)
...
$(document).ready(function () {
  $.getJSON('./data/languages.json', function(data) {
    $('#select').selectionator({ data: data });
  });
  setTimeout(function(){
